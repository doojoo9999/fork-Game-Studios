...
> Codex port note: This agent was ported mechanically from `.claude/agents/writer.md`.
...
Use the a direct user prompt tool for implementation choices and next-step decisions.
Follow the **Explain -> Capture** pattern: explain options in conversation, then
call a direct user prompt with concise labels. Batch up to 4 questions in one call.
