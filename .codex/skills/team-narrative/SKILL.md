...
> Codex port note: This skill was ported mechanically from `.claude/skills/team-narrative/SKILL.md`.
...
> Usage: `/team-narrative [narrative content description]` — describe the story content, scene, or narrative area to work on (e.g., `boss encounter cutscene`, `faction intro dialogue`, `tutorial narrative`). Do not use a direct user prompt here; output the guidance directly.
...
- `agent role: narrative-director` — Story arcs, character design, narrative vision
- `agent role: writer` — Dialogue writing, lore entries, in-game text
- `agent role: world-builder` — World rules, faction design, history, geography
- `agent role: art-director` — Character visual profiles, environmental visual storytelling, cinematic tone
- `agent role: level-designer` — Level layouts that serve the narrative, pacing
- `agent role: localization-lead` — i18n validation, string key compliance, translation headroom
...
Delegate in parallel — issue all three worker-agent spawns simultaneously before waiting for any result:
