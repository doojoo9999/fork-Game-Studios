...
> Codex port note: This skill was ported mechanically from `.claude/skills/team-audio/SKILL.md`.
...
> Usage: `/team-audio [feature or area]` — specify the feature or area to design audio for (e.g., `combat`, `main menu`, `forest biome`, `boss encounter`). Do not use a direct user question here; output the guidance directly.
...
- `agent role: audio-director` — Sonic identity, emotional tone, audio palette
- `agent role: sound-designer` — SFX specifications, audio events, mixing groups
- `agent role: technical-artist` — Audio middleware, bus structure, memory budgets
- `agent role: [primary engine specialist]` — Validate audio integration patterns for the engine
- `agent role: gameplay-programmer` — Audio manager, gameplay triggers, adaptive music
