...
> Codex port note: This skill was ported mechanically from `.claude/skills/team-release/SKILL.md`.
...
2. If a version is found: report "No version argument provided — inferred [version] from milestone data. Proceeding." Then confirm with a direct user prompt: "Releasing [version]. Is this correct?"
3. If no version is discoverable: use a direct user prompt to ask "What version number should be released? (e.g., v1.0.0)" and wait for user input before proceeding. Do NOT default to a hardcoded version string.
...
Use Codex multi-agent tools to spawn each team member as a subagent:
- `agent role: release-manager` — Release branch, versioning, changelog, deployment
- `agent role: qa-lead` — Test sign-off, regression suite, release quality gate
- `agent role: devops-engineer` — Build pipeline, artifacts, deployment automation
- `agent role: security-engineer` — Security audit for online/multiplayer/data features
- `agent role: analytics-engineer` — Telemetry event verification and dashboard readiness
- `agent role: community-manager` — Patch notes and launch communication
- `agent role: producer` — Go/no-go decision, stakeholder communication
- `agent role: network-programmer` — Netcode stability sign-off (invoke if game has multiplayer)
