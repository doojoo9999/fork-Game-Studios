...
> Codex port note: This skill was ported mechanically from `.claude/skills/hotfix/SKILL.md`.
...
Use Codex multi-agent tools to request sign-off in parallel:

- `agent role: lead-programmer` — Review the fix for correctness and side effects
- `agent role: qa-tester` — Run targeted regression tests on the affected system
- `agent role: producer` — Approve deployment timing and communication plan
...
After approvals, determine the QA scope required before deploying the hotfix. Spawn `qa-lead` via Codex multi-agent tools with:
