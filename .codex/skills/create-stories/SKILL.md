...
> Codex port note: This skill was ported mechanically from `.claude/skills/create-stories/SKILL.md`.
...
After decomposing all stories (Step 4 complete) but before presenting them for write approval, spawn `qa-lead` via Codex multi-agent tools using gate **QL-STORY-READY** (`.claude/docs/director-gates.md`).
...
Use a direct user question to close with context-aware next steps:
