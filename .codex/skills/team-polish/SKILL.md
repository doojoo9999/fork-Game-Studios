...
> Codex port note: This skill was ported mechanically from `.claude/skills/team-polish/SKILL.md`.
...
> Usage: `/team-polish [feature or area]` — specify the feature or area to polish (e.g., `combat`, `main menu`, `inventory system`, `level-1`). Do not use a direct user question here; output the guidance directly.
...
- `agent role: performance-analyst` — Profiling, optimization, memory analysis
- `agent role: engine-programmer` — Engine-level fixes for rendering, memory, resource loading
- `agent role: technical-artist` — VFX polish, shader optimization, visual quality
- `agent role: sound-designer` — Audio polish, mixing, ambient layers
- `agent role: tools-programmer` — Content pipeline and editor tool verification
- `agent role: qa-tester` — Edge case testing, regression testing, soak testing
