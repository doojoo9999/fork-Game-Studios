...
> Codex port note: This skill was ported mechanically from `.claude/skills/team-combat/SKILL.md`.
...
- `agent role: game-designer` — Design the mechanic, define formulas and edge cases
- `agent role: gameplay-programmer` — Implement the core gameplay code
- `agent role: ai-programmer` — Implement NPC/enemy AI behavior
- `agent role: technical-artist` — Create VFX, shader effects, visual feedback
- `agent role: sound-designer` — Define audio events, impact sounds, ambient audio
- `agent role: [primary engine specialist]` — Engine idiom validation for architecture and implementation
- `agent role: qa-tester` — Write test cases and validate implementation
...
delegated to sub-agents spawned via Codex multi-agent tools. Each sub-agent enforces the
