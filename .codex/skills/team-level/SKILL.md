...
- `agent role: narrative-director` — Narrative purpose, characters, emotional arc
- `agent role: world-builder` — Lore context, environmental storytelling, world rules
- `agent role: level-designer` — Spatial layout, pacing, encounters, navigation
- `agent role: systems-designer` — Enemy compositions, loot tables, difficulty balance
- `agent role: art-director` — Visual theme, color palette, lighting, asset requirements
- `agent role: accessibility-specialist` — Navigation clarity, colorblind safety, cognitive load
- `agent role: qa-tester` — Test cases, boundary testing, playtest checklist
