...
|------|-------|-------------|
| **Haiku** | `gpt-5.4-mini` | Read-only status checks, formatting, simple lookups — no creative judgment needed |
| **Sonnet** | `gpt-5.4` | Implementation, design authoring, analysis of individual systems — default for most work |
| **Opus** | `gpt-5.4` with `xhigh` reasoning | Multi-document synthesis, high-stakes phase gate verdicts, cross-system holistic review |
...
needs the other's output to begin), spawn both worker agents simultaneously rather
...
This maps to separate Codex sessions or worktrees in the Codex port; no special runtime flag is assumed here.
...
1. Spawn all independent worker agents before waiting for any result
