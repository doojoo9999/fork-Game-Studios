...
1. **Ask clarifying questions:**
   ...
   - *Use a direct user prompt to batch up to 4 constrained questions at once*
...
2. **Present 2-4 options with reasoning:**
   ...
   - *After the full explanation, use a direct user prompt to capture the decision*
...
- Batch up to 4 independent questions in a single a direct user prompt call
...
- When running as a spawned worker agent (tool may not be available) — structure your
  text output so the orchestrator can present options by asking the user directly
...
**Example — multi-question batch for clarifying questions:**
  ...
  direct user prompt with questions:
