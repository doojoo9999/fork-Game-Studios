...
Spawn all [N] agents simultaneously via `spawn_agent` — issue all worker-agent spawns before
...
| **CONCERNS [list]** | Issues present but not blocking. | Surface to user via a direct user prompt — options: `Revise flagged items` / `Accept and proceed` / `Discuss further` |
...
```
Spawn in parallel (issue all worker-agent spawns before waiting for any result):
